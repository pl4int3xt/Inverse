...
class GetGamesByCategoryUseCase @Inject constructor(
    ...
    operator fun invoke(genre: String ,page: Int, pageSize: Int): Flow<Resource<List<GameModel>>> = flow {
        try {
            emit(Resource.Loading())
            val games = repository.getGamesByCategory(page, pageSize, genre).results.map { it.toGameModel() }
